...
@agents.agent(
    name="Researcher",
    instruction="""
You are a research assistant, with access to internet search (via searxng),
...
You must always provide a summary of the specific sources you have used in your research.
    """,
    servers=["mcp-searxng", "interpreter", "filesystem", "fetch"],
)
@agents.agent(
    name="Evaluator",
    instruction="""
...
async def main() -> None:
    async with agents.run() as agent:
        await agent.Researcher_Evaluator.send("What are dogs?")
